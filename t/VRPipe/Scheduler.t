...
ok my $scheduler = VRPipe::Scheduler->get, 'able to get a VRPipe::Scheduler based on configured default';

# local
ok $scheduler = VRPipe::Scheduler->get(type => 'local'), q[able to get the lsf scheduler using get(type => 'local')];
is $scheduler->type, 'local', 'the type really is local';

my $requirements = VRPipe::Requirements->get(memory => 1, time => 1);
is $scheduler->determine_queue($requirements), 'local', q[determine_queue() returned local's only queue];


# lsf
ok $scheduler = VRPipe::Scheduler->get(type => 'lsf'), q[able to get the lsf scheduler using get(type => 'lsf')];
