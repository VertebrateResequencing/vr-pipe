...
is_deeply [$file_list->id, ($file_list->files)[0]->id, ($file_list->files)[1]->id], [2, $files[0]->id, $files[1]->id], 'the created FileList has a new id, but the same contents otherwise';
ok $file_list = VRPipe::FileList->get(files => \@files), 'got a FileList using a set of files';
is $file_list->id, 1, 'it had the id of the first matching FileList';
...
is_deeply [$file_list->id, scalar($file_list->files)], [3, 0], 'the created FileList has a new id, and no files';
...
undef $keyval_list;
ok $keyval_list = VRPipe::KeyValList->get(id => 1), 'got a KeyValList using get(id => 1)';
is_deeply [$keyval_list->id, ($keyval_list->keyvals)[0]->id, ($keyval_list->keyvals)[1]->id, $keyval_list->as_hashref], [1, 1, 2, $test_hash], 'the gotten KeyValList has the correct contents';
ok $keyval_list = VRPipe::KeyValList->create(keyvals => [$keyval_list->keyvals]), 'created a KeyValList using the same set of keyvals)';
is_deeply [$keyval_list->id, ($keyval_list->keyvals)[0]->id, ($keyval_list->keyvals)[1]->id, $keyval_list->as_hashref], [2, 1, 2, $test_hash], 'the created FileList has a new id, but the same contents otherwise';
ok $keyval_list = VRPipe::KeyValList->get(keyvals => [$keyval_list->keyvals]), 'got a KeyValList using a set of keyvals';
is $keyval_list->id, 1, 'it had the id of the first matching KeyValList';
...
is $defaulted_scheduler->id, $default_scheduler_id, 'The scheduler was the default one';
