...
my $dict     = file(qw(t data human_g1k_v37.fasta.dict))->absolute->stringify;
my $override = file(qw(t data wgs_calling_override_options))->absolute->stringify;

# genome chunking
my $chunks = [{ chrom => 11, from => 1, to => 10000000, chunk_id => 1, chunk_override_file => $override }, { chrom => 11, from => 10000001, to => 20000000, chunk_id => 2, chunk_override_file => $override }, { chrom => 11, from => 20000001, to => 30000000, chunk_id => 3, chunk_override_file => $override }, { chrom => 11, from => 30000001, to => 40000000, chunk_id => 4, chunk_override_file => $override }, { chrom => 11, from => 40000001, to => 50000000, chunk_id => 5, chunk_override_file => $override }, { chrom => 11, from => 50000001, to => 60000000, chunk_id => 6, chunk_override_file => $override }, { chrom => 11, from => 60000001, to => 70000000, chunk_id => 7, chunk_override_file => $override }, { chrom => 11, from => 70000001, to => 80000000, chunk_id => 8, chunk_override_file => $override }, { chrom => 11, from => 80000001, to => 90000000, chunk_id => 9, chunk_override_file => $override }, { chrom => 11, from => 90000001, to => 100000000, chunk_id => 10, chunk_override_file => $override }, { chrom => 11, from => 100000001, to => 110000000, chunk_id => 11, chunk_override_file => $override }, { chrom => 11, from => 110000001, to => 120000000, chunk_id => 12, chunk_override_file => $override }, { chrom => 11, from => 120000001, to => 130000000, chunk_id => 13, chunk_override_file => $override }, { chrom => 11, from => 130000001, to => 135006516, chunk_id => 14, chunk_override_file => $override }, { chrom => 20, from => 1, to => 10000000, chunk_id => 15, chunk_override_file => $override }, { chrom => 20, from => 10000001, to => 20000000, chunk_id => 16, chunk_override_file => $override }, { chrom => 20, from => 20000001, to => 30000000, chunk_id => 17, chunk_override_file => $override }, { chrom => 20, from => 30000001, to => 40000000, chunk_id => 18, chunk_override_file => $override }, { chrom => 20, from => 40000001, to => 50000000, chunk_id => 19, chunk_override_file => $override }, { chrom => 20, from => 50000001, to => 60000000, chunk_id => 20, chunk_override_file => $override }, { chrom => 20, from => 60000001, to => 63025520, chunk_id => 21, chunk_override_file => $override }];

ok $ds = VRPipe::DataSource->create(type    => 'fofn_with_genome_chunking',
                                    method  => 'group_all',
                                    source  => file(qw(t data datasource.fofn))->absolute->stringify,
                                    options => { reference_dict => $dict, chunk_override_file => $override, chrom_list => '11 20', chunk_size => 10000000 }),
...
$chunks = [{ chrom => 'X', from => 1, to => 60000, male_ploidy => 1, female_ploidy => 2, chunk_id => 1, chunk_override_file => $override }, { chrom => 'X', from => 60001, to => 2699520, male_ploidy => 2, female_ploidy => 2, chunk_id => 2, chunk_override_file => $override }, { chrom => 'X', from => 2699521, to => 154931043, male_ploidy => 1, female_ploidy => 2, chunk_id => 3, chunk_override_file => $override }, { chrom => 'X', from => 154931044, to => 155270560, male_ploidy => 2, female_ploidy => 2, chunk_id => 4, chunk_override_file => $override }, { chrom => 'Y', from => 1, to => 59373566, male_ploidy => 1, female_ploidy => 0, chunk_id => 5, chunk_override_file => $override }, { chrom => 'MT', from => 1, to => 16569, male_ploidy => 1, female_ploidy => 1, chunk_id => 6, chunk_override_file => $override }];

ok $ds = VRPipe::DataSource->create(type    => 'fofn_with_genome_chunking',
                                    method  => 'all',
                                    source  => file(qw(t data datasource.fofn))->absolute->stringify,
                                    options => { reference_dict => $dict, chunk_override_file => $override, chrom_list => 'X Y MT', chunk_size => 155000000, ploidy => file(qw(t data ploidy_definition))->absolute->stringify }),
