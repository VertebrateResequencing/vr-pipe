...
VRPipe::PipelineSetup->get(name => 'wgs test mapping',
                           ...
                           options => { genomic_region_file => $ref_fa.'.fai',
...
is_deeply [VRPipe::StepState->get(pipelinesetup => 1, stepmember => 3, dataelement => 1)->cmd_summary->summary,
          ...
          ['samtools mpileup -EDSV -C50 -m2 -F0.0005 -d 10000 -P ILLUMINA -g -r $region -f $reference_fasta $bam_files > $bcf_file',
...
is_deeply [VRPipe::File->get(path => $final_files[0])->metadata,
          ...
          [{ chrom => '11',
           ...
           { chrom => '20',
             analysis_group => 'low_coverage' }], 'final merged vcfs have correct metadata';
