...
ok my $calling_pipeline = VRPipe::Pipeline->get(name => 'snp_calling_mpileup_vcf'), 'able to get the snp_calling_mpileup_vcf pipeline';
...
foreach my $sample (qw(1 2)) {
    foreach my $chunk (@$chunks) {
        ...
        push(@calling_files, file(@output_subdirs, '1_mpileup_vcf', qq[$region.mpileup.vcf.gz]));
        push(@calling_files, file(@output_subdirs, '1_mpileup_vcf', qq[$region.mpileup.vcf.gz.tbi]));
        ++$element_id;
    }
}
...
is_deeply [VRPipe::StepState->get(pipelinesetup => 1, stepmember => 1, dataelement => 1)->cmd_summary->summary], ['samtools mpileup -DSV -C50 -m2 -F0.0005 -d 10000 -ug -r $region -f $reference_fasta $bam_files | bcftools view -p 0.99 -vcgN -s $samples_file - | bgzip -c > $vcf_file'], 'cmd summaries for the major steps were as expected';
