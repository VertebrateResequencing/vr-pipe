...
my $mapping_output_dir = get_output_dir('454_mapping');

ok my $mapping_pipeline = VRPipe::Pipeline->get(name => 'fastq_mapping_with_smalt'), 'able to get a pre-written pipeline';
my @s_names;
foreach my $stepmember ($mapping_pipeline->steps) {
    push(@s_names, $stepmember->step->name);
}
is_deeply \@s_names, [qw(sequence_dictionary smalt_index fastq_import fastq_metadata fastq_split smalt_map_to_sam sam_to_fixed_bam bam_merge_lane_splits bam_stats)], 'the pipeline has the correct steps';
...
my $mapping_output_dir2 = get_output_dir('454_mapping_cleanup');
...
is_deeply [VRPipe::StepState->get(pipelinesetup => 1, stepmember => 2, dataelement => 1)->cmd_summary->summary,
          ...
          ['smalt index -k 13 -s 4 $index_base $reference_fasta',
           'smalt map -f samsoft -o $sam_file $index_base $fastq_file(s)',
