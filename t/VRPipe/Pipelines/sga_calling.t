...
while (my ($sample, $element_id) = each %samples) {
    ...
    foreach my $chrom (qw(11 20)) {
        ...
        push(@input_files, file(@output_subdirs, '3_bam_to_fastq',          qq[chrom$chrom.${sample}_1.fastq]));
        push(@input_files, file(@output_subdirs, '3_bam_to_fastq',          qq[chrom$chrom.${sample}_2.fastq]));
        push(@input_files, file(@output_subdirs, '4_sga_preprocess',        qq[chrom$chrom.${sample}.processed.fq.gz]));
    }
}
