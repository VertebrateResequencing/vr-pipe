...
BEGIN {
    ...
    use VRPipeTest (
        ...
        required_exe => [qw(samtools bcftools freebayes)]
    );
    use TestPipelines;
}

ok my $calling_pipeline = VRPipe::Pipeline->create(name => 'bam_calling_with_freebayes'), 'able to get the bam_calling_with_freebayes pipeline';

my $calling_dir = get_output_dir('freebayes');
...
VRPipe::PipelineSetup->create(
    ...
    options     => {
        ...
        post_processing         => 'bcftools view -Ob > $output_bcf',
        chrom_list              => '11 20',
        chunk_size              => 0,
        chunk_overlap           => 0,
...
ok handle_pipeline(), 'freebayes pipelines ran ok';
