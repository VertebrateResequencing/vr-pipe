...
cmp_deeply [VRPipe::StepState->create(pipelinesetup => 1, stepmember => 2, dataelement => 1)->cmd_summary->summary, VRPipe::StepState->create(pipelinesetup => 1, stepmember => 6, dataelement => 1)->cmd_summary->summary, VRPipe::StepState->create(pipelinesetup => 1, stepmember => 7, dataelement => 1)->cmd_summary->summary, VRPipe::StepState->create(pipelinesetup => 1, stepmember => 8, dataelement => 1)->cmd_summary->summary, VRPipe::StepState->create(pipelinesetup => 2, stepmember => 11, dataelement => 5)->cmd_summary->summary, VRPipe::StepState->create(pipelinesetup => 2, stepmember => 12, dataelement => 5)->cmd_summary->summary, VRPipe::StepState->create(pipelinesetup => 3, stepmember => 13, dataelement => 8)->cmd_summary->summary], ['bwa index -a is $reference_fasta', 'bwa aln -q 15 -f $sai_file $reference_fasta $fastq_file', 'bwa sampe -a 600 -r $rg_line -f $sam_file $reference_fasta $sai_file(s) $fastq_file(s)', re('samtools view -bSu \$sam_file | samtools sort -n -o - samtools_nsort_tmp | samtools fixmate( -O bam)? /dev/stdin /dev/stdout | samtools sort -o - samtools_csort_tmp | samtools fillmd -u - \$reference_fasta > \$fixed_bam_file'), re('java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT'), re('java \$jvm_args -jar .*MarkDuplicates(\.jar)? INPUT=\$bam_file OUTPUT=\$markdup_bam_file ASSUME_SORTED=TRUE METRICS_FILE=/dev/null VALIDATION_STRINGENCY=SILENT'), re('java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT')], 'cmd summaries for the major steps were as expected';

my $samtools_v1 = 0;
if ($samtools_version =~ /^(\d)/) {
    if ($1 > 0) {
        $samtools_v1 = 1;
    }
}

my @expected_header_lines = (
    ...
    re("\@PG\tID:sam_to_fixed_bam\tPN:samtools\tPP:bwa_sam\tVN:$samtools_version\tCL:samtools view -bSu \$sam_file \| samtools sort -n -o - samtools_nsort_tmp \| samtools fixmate( -O bam)? /dev/stdin /dev/stdout \| samtools sort -o - samtools_csort_tmp \| samtools fillmd -u - \$reference_fasta > \$fixed_bam_file"),
    re("\@PG\tID:sam_to_fixed_bam.1\tPN:samtools\tPP:bwa_sam.1\tVN:$samtools_version\tCL:samtools view -bSu \$sam_file \| samtools sort -n -o - samtools_nsort_tmp \| samtools fixmate( -O bam)? /dev/stdin /dev/stdout \| samtools sort -o - samtools_csort_tmp \| samtools fillmd -u - \$reference_fasta > \$fixed_bam_file"),
    re("\@PG\tID:sam_to_fixed_bam.2\tPN:samtools\tPP:bwa_sam.2\tVN:$samtools_version\tCL:samtools view -bSu \$sam_file \| samtools sort -n -o - samtools_nsort_tmp \| samtools fixmate( -O bam)? /dev/stdin /dev/stdout \| samtools sort -o - samtools_csort_tmp \| samtools fillmd -u - \$reference_fasta > \$fixed_bam_file"),
    $samtools_v1
    ? (
        "\@PG\tID:bam_merge_lane_splits\tPN:samtools\tPP:sam_to_fixed_bam\tVN:$samtools_version\tCL:samtools merge -cp -h \$header_file \$output_file \@bam_files",
        "\@PG\tID:bam_merge_lane_splits.1\tPN:samtools\tPP:sam_to_fixed_bam.1\tVN:$samtools_version\tCL:samtools merge -cp -h \$header_file \$output_file \@bam_files",
        "\@PG\tID:bam_merge_lane_splits.2\tPN:samtools\tPP:sam_to_fixed_bam.2\tVN:$samtools_version\tCL:samtools merge -cp -h \$header_file \$output_file \@bam_files",
        re(qr/\@PG\tID:bam_merge\tPN:picard\tPP:bam_merge_lane_splits\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/),
        re(qr/\@PG\tID:bam_merge.1\tPN:picard\tPP:bam_merge_lane_splits.1\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/),
        re(qr/\@PG\tID:bam_merge.2\tPN:picard\tPP:bam_merge_lane_splits.2\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/)
      )
    : (
        re(qr/\@PG\tID:bam_merge\tPN:picard\tPP:sam_to_fixed_bam\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/),
        re(qr/\@PG\tID:bam_merge.1\tPN:picard\tPP:sam_to_fixed_bam.1\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/),
        re(qr/\@PG\tID:bam_merge.2\tPN:picard\tPP:sam_to_fixed_bam.2\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/)
    ),
    re(qr/\@PG\tID:bam_mark_duplicates\tPN:picard\tPP:bam_merge\tVN:$picard_version\tCL:java \$jvm_args -jar .*MarkDuplicates(\.jar)? INPUT=\$bam_file OUTPUT=\$markdup_bam_file ASSUME_SORTED=TRUE METRICS_FILE=\/dev\/null VALIDATION_STRINGENCY=SILENT/),
    re(qr/\@PG\tID:bam_mark_duplicates.1\tPN:picard\tPP:bam_merge.1\tVN:$picard_version\tCL:java \$jvm_args -jar .*MarkDuplicates(\.jar)? INPUT=\$bam_file OUTPUT=\$markdup_bam_file ASSUME_SORTED=TRUE METRICS_FILE=\/dev\/null VALIDATION_STRINGENCY=SILENT/),
    re(qr/\@PG\tID:bam_mark_duplicates.2\tPN:picard\tPP:bam_merge.2\tVN:$picard_version\tCL:java \$jvm_args -jar .*MarkDuplicates(\.jar)? INPUT=\$bam_file OUTPUT=\$markdup_bam_file ASSUME_SORTED=TRUE METRICS_FILE=\/dev\/null VALIDATION_STRINGENCY=SILENT/),
    re(qr/\@PG\tID:bam_merge.3\tPN:picard\tPP:bam_mark_duplicates\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/),
    re(qr/\@PG\tID:bam_merge.1.3\tPN:picard\tPP:bam_mark_duplicates.1\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/),
    re(qr/\@PG\tID:bam_merge.2.3\tPN:picard\tPP:bam_mark_duplicates.2\tVN:$picard_version\tCL:java \$jvm_args -jar .*MergeSamFiles(\.jar)? INPUT=\$bam_file\(s\) OUTPUT=\$merged_bam VALIDATION_STRINGENCY=SILENT/)
);

my @header_lines = get_bam_header($split_files[0]);
cmp_deeply \@header_lines, \@expected_header_lines, 'split bam header is okay';
