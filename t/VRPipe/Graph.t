...
ok $graph->add_schema(namespace => 'VRTrack', label => 'Sample', unique => [qw(sanger_id uuid)], indexed => [qw(public_name)]), 'add_schema worked';
is $graph->add_schema(namespace => 'VRTrack', label => 'Sample', unique => [qw(sanger_id uuid)], indexed => [qw(public_name)]), 0, 'add_schema a second time with same args does nothing';
...
throws_ok { $graph->add_node(namespace => 'VRTrack', label => 'Sample', properties => { sanger_id => 'sanger1', public_name => 'public2' }) } qr/already exists with label VRTrack\|Sample/, 'add_node throws when used twice with the same unique arg but different other arg';
...
$node = $graph->add_node(namespace => 'OtherNS', label => 'Individual', properties => { name => 'Sam' });
