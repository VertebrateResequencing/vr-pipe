...
BEGIN {
    ...
    use VRPipeTest (
        required_env => [qw(VRPIPE_TEST_PIPELINES TRIMMOMATIC TRIMMOMATIC_LOG_PATH)] #require env TRIMMOMATIC ?
...
my $setup = VRPipe::PipelineSetup->create(name        => 'trimmomatic',
                                          ...
                                          pipeline    => $pipeline,
                                          options     => { trimmomatic_jar_path => $ENV{TRIMMOMATIC}, trimmomatic_step_options => 'LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36', log_file => $ENV{TRIMMOMATIC_LOG_PATH} });
