...
VRPipe::Utils::Graph - graph drawing functions

=head1 SYNOPSIS

use VRPipe::Utils::Graph;

Example of plot_stats:
...
Example of plot_histograms_distributions:
...
General graph drawing functions for plotting stats and histograms.
...
=head2 plot_stats
  ...
  Title		: plot_stats
  Usage		: Graphs::plot_stats({outfile=>"$outfiles_prefix.coverage_per_base.$plot_type",
                                  title => "Bait and target coverage per base",
                                  desc_xvals => 'Coverage',
                                  desc_yvals => 'Frequency',
                                  data => \@data});
  Function	: Create graphs for the given data sets.	
  Returns	: None	
  Args		: The statistics collected by e.g. SamTools::collect_detailed_bam_stats. 
...
=cut
	...
	method plot_stats (HashRef $stats) {

    	$self->throw("File for output not specified\n") unless exists($$stats{'outfile'});
    	$self->throw("Data for graph not provided\n") unless exists($$stats{'data'});
    	$self->throw("Could not determine the filetype of $$stats{'outfile'}\n") unless $$stats{'outfile'} =~ /([^.]+)$/;
    	my $file_type = $1;
my $msg = "Back atcha....";
    	...
    	if ( $barplot and @{ $$stats{'data'} } > 1 ) {
    		$self->throw("Multiple barplots in one graph.\n");
    	}
    	open(my $ofh, '>', "$$stats{'outfile'}.R") or $self->throw("$$stats{'outfile'}.R: $!");
		
		print $ofh "par(bg='cornsilk')\n";
    	print $ofh "$file_type(file='$$stats{'outfile'}')\n";

    	my %legend;
    	my $set  = 0;
    	for my $vals ( @{$$stats{'data'}} ) {
        	$self->throw("Given empty data set for $$stats{'outfile'}\n") unless ( $vals && scalar @{$$vals{'xvals'}} ); 
        	if ( exists($$vals{legend}) ) { 
        		push @{$legend{label}}, qq["$$vals{legend}"]; 
        	}
    	...
    	print $ofh "par(cex=1.25)\n";
    	if ( $barplot ) {
        	print $ofh "par(las=2)\n";
        print $ofh "barplot(y0,,,x0, xlab='$xlabel',ylab='$ylabel')\n";
    	}
    	else {
        	my $r_plot = exists($$stats{'r_plot'}) ? ", $$stats{'r_plot'}" : '';
        	print $ofh "plot(xrange,yrange,type='n', xlab='$xlabel',ylab='$ylabel' $r_plot)\n";
        	$set = 0;
        	for my $vals ( @{$$stats{'data'}} ) {
            	...
            	print $ofh "lines(x$set,y$set,$type,col=",$set+1,",pch=",$set+1,"$lines)\n";
            	$set++;
        	}
    	}
    	print $ofh "title(main='$title', font.main=3)\n";
    	print $ofh $$stats{'r_cmd'} unless !exists($$stats{'r_cmd'});
    	...
    	my $cmd_line = "cat $$stats{'outfile'}.R | $R_CMD --slave --vanilla";
    	system($cmd_line) && $self->throw("failed to run [$cmd_line]");
    	return $msg;
	}

=head2 plot_histograms_distributions
  Title		: plot_histograms_distributions
  Usage		: Graphs::plot_histograms_distributions({outfile=>"$outfiles_prefix.$_" . "_gc_vs_cvg.$plot_type",
                                                     title => "Coverage vs GC plot of $_" . 's',
                                               	     desc_xvals => 'GC (%)',
                                                     desc_yvals => 'Mapped depth',
                                                     xdata => [(0..100)],
                                                     ydata => $stats->{"gc_vs_$_" . '_cvg'}});
  Function	: Create plot from an array of histograms, showing quartiles and means	
  Returns	: None
  Args      : Hash reference with the following keys:
                            outfile            .. file types supported by R: png, pdf, or jpg
...
=cut
	...
	method plot_histograms_distributions (HashRef $hash_in) {
		my @quartiles1;
    	my @quartiles2;
    	my @quartiles3;
    	my @means;
    	...
    	my @xvals;
    	my $math_util = VRPipe::Utils::Math->new();
    	$self->throw("Could not determine the filetype of \"$hash_in->{outfile}\"\n") unless ($hash_in->{outfile}=~ /([^.]+)$/);
    	my $filetype = $1;
    	...
    	foreach my $i (0 .. (scalar @{$hash_in->{xdata}} - 1)) {
        	...
        	my %stats = $math_util->histogram_stats($hash_in->{ydata}->[$i]);
    	...
    	open my $ofh, '>', "$hash_in->{outfile}.R" or Utils::error("$hash_in->{outfile}.R: $!");
        #my $graph_out = VRPipe::File->get(path => "$hash_in->{outfile}.R"); 
        #my $ofh = $graph_out->openw;
        #$graph_out->disconnect;
     	print $ofh qq[ $filetype("$hash_in->{outfile}")
                 q1 = $q1_string
                 q2 = $q2_string
                 q3 = $q3_string
                 x = $xvals_string
       	...
       	#$graph_out->close;
       	close $ofh;
       	my $cmd_line = "cat $hash_in->{outfile}.R | $R_CMD --slave --vanilla";
       	system($cmd_line) && $self->throw("failed to run [$cmd_line]");
    	return;
	}
