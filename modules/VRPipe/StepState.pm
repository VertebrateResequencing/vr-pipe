    ...
    method output_files (Maybe[PersistentFileHashRef] $new_hash?, Bool :$only_unique_to_us?) {
        ...
        foreach my $sof (@current_sofiles) {
            ...
            if ($only_unique_to_us) {
                my $others = VRPipe::StepOutputFile->search({ file => $file->id, output_key => { '!=' => 'temp' }, stepstate => { '!=' => $self->id }, -and => [pipelinesetup => { '!=' => $self->pipelinesetup->id }] }, { prefetch => 'stepstate', join => { stepstate => 'pipelinesetup' } });
                next if $others;
            }
