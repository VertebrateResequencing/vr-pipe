...
class VRPipe::Pipelines::bam_merge_1000_genomes_libraries_with_split with VRPipe::PipelineRole {
    ...
    method adaptor_definitions {
        (
            ...
            { from_step => 1, to_step => 3, from_key => 'merged_bam_files', to_key => 'bam_files' },
            { from_step => 2, to_step => 3, from_key => 'reference_dict', to_key => 'dict_file' },
            { from_step => 3, to_step => 4, from_key => 'headed_bam_files', to_key => 'bam_files' },
        );
    }
    ...
    method behaviour_definitions {
        (
            ...
            { after_step => 4, behaviour => 'delete_outputs', act_on_steps => [3], regulated_by => 'remove_merged_bams', default_regulation => 0 }
        );
    }
}
