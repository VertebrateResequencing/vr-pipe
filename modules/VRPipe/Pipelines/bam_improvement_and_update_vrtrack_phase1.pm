    ...
    method _step_list {
        return ([ VRPipe::Step->get(name => 'sequence_dictionary'),#1
                ...
                [ VRPipe::StepAdaptorDefiner->new(from_step => 0, to_step => 2, to_key => 'bam_files'),
                  ...
                  VRPipe::StepAdaptorDefiner->new(from_step => 7, to_step => 8, from_key => 'fixmate_bam_files', to_key => 'bam_files'),
                  VRPipe::StepAdaptorDefiner->new(from_step => 7, to_step => 9, from_key => 'fixmate_bam_files', to_key => 'bam_files'),
                  VRPipe::StepAdaptorDefiner->new(from_step => 9, to_step => 10, from_key => 'bam_recalibration_files', to_key => 'bam_recalibration_files'),
                  VRPipe::StepAdaptorDefiner->new(from_step => 7, to_step => 10, from_key => 'fixmate_bam_files', to_key => 'bam_files'),
                  VRPipe::StepAdaptorDefiner->new(from_step => 10, to_step => 11, from_key => 'recalibrated_bam_files', to_key => 'bam_files'),
                  VRPipe::StepAdaptorDefiner->new(from_step => 11, to_step => 12, from_key => 'bq_bam_files', to_key => 'bam_files'),
                  VRPipe::StepAdaptorDefiner->new(from_step => 1, to_step => 12, from_key => 'reference_dict', to_key => 'dict_file'),
                  VRPipe::StepAdaptorDefiner->new(from_step => 12, to_step => 13, from_key => 'headed_bam_files', to_key => 'bam_files')
                  ],
                ...
                [ VRPipe::StepBehaviourDefiner->new(after_step => 5, behaviour => 'delete_inputs', act_on_steps => [0], regulated_by => 'delete_input_bams', default_regulation => 0),
                  VRPipe::StepBehaviourDefiner->new(after_step => 5, behaviour => 'delete_outputs', act_on_steps => [3], regulated_by => 'delete_input_bams', default_regulation => 0),
                  VRPipe::StepBehaviourDefiner->new(after_step => 8, behaviour => 'delete_outputs', act_on_steps => [5,6], regulated_by => 'cleanup', default_regulation => 1),
                  VRPipe::StepBehaviourDefiner->new(after_step => 10, behaviour => 'delete_outputs', act_on_steps => [7,8,9], regulated_by => 'cleanup', default_regulation => 1),
