...
VRPipe::Pipelines::gvcf_calling_one_combine_with_gatk_haplotype_caller_with_vep
- a pipeline
...
class VRPipe::Pipelines::gvcf_calling_one_combine_with_gatk_haplotype_caller_with_vep with VRPipe::PipelineRole {
    method name {
        return 'gvcf_calling_one_combine_with_gatk_haplotype_caller_with_vep';
    }
    
    method description {
        return 'Run GATK to combine and genotype gVCF files and run Variant Effect Predictor (VEP). Use this pipeline for up to 40,000 samples.';
    }
    ...
    method adaptor_definitions {
        (
            ...
            { from_step => 2, to_step => 3, from_key => 'genotype_gvcf_file', to_key => 'vcf_files' },
            { from_step => 3, to_step => 4, from_key => 'vep_annot_vcf_files', to_key => 'vcf_files' },
        );
    }
