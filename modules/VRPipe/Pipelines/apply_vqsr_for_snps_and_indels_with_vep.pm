...
VRPipe::Pipelines::apply_vqsr_for_snps_and_indels_with_vep - a pipeline
...
class VRPipe::Pipelines::apply_vqsr_for_snps_and_indels_with_vep with VRPipe::PipelineRole {
    method name {
        return 'apply_vqsr_for_snps_and_indels_with_vep';
    }
    
    method description {
        return 'Apply VQSR to SNPs and indels and run Variant Effect Predictor (VEP).';
    }
    ...
    method step_names {
        (
            ...
            'vep_annotate',                        #3
            'bcftools_concat',                     #4 make per-chromosome, VQSR applied vcf files
        );
    }
