...
class VRPipe::Pipelines::vcf_combine_genotypes_and_sites with VRPipe::PipelineRole {
    ...
    method _step_list {
        return ([
                ...
                VRPipe::Step->get(name => 'vcf_combine_genotypes_and_sites'), #2
                VRPipe::Step->get(name => 'vcf_index'),                       #3
            ],
            [VRPipe::StepAdaptorDefiner->new(from_step => 0, to_step => 1, to_key => 'vcf_files'), VRPipe::StepAdaptorDefiner->new(from_step => 0, to_step => 2, to_key => 'vcf_files'), VRPipe::StepAdaptorDefiner->new(from_step => 2, to_step => 3, from_key => 'combined_vcf', to_key => 'vcf_files')],
