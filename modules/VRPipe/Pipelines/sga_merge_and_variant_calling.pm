...
class VRPipe::Pipelines::sga_merge_and_variant_calling with VRPipe::PipelineRole {
    ...
    method _step_list {
        return ([
            ...
            [VRPipe::StepBehaviourDefiner->new(after_step => 4, behaviour => 'delete_inputs', act_on_steps => [0], regulated_by => 'remove_inputs', default_regulation => 0), VRPipe::StepBehaviourDefiner->new(after_step => 5, behaviour => 'delete_outputs', act_on_steps => [4], regulated_by => 'cleanup', default_regulation => 1), VRPipe::StepBehaviourDefiner->new(after_step => 6, behaviour => 'delete_outputs', act_on_steps => [5], regulated_by => 'cleanup', default_regulation => 1), VRPipe::StepBehaviourDefiner->new(after_step => 7, behaviour => 'delete_outputs', act_on_steps => [6], regulated_by => 'cleanup', default_regulation => 1), VRPipe::StepBehaviourDefiner->new(after_step => 8, behaviour => 'delete_outputs', act_on_steps => [7], regulated_by => 'remove_final_merged_fastq', default_regulation => 0),]
        );
    }
}
