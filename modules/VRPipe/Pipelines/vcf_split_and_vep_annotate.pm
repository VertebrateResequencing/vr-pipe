...
class VRPipe::Pipelines::vcf_split_and_vep_annotate with VRPipe::PipelineRole {
    ...
    method _step_list {
        return ([
            ...
            [VRPipe::StepAdaptorDefiner->new(from_step => 0, to_step => 1, to_key => 'vcf_files'), VRPipe::StepAdaptorDefiner->new(from_step => 0, to_step => 2, to_key => 'vcf_files'), VRPipe::StepAdaptorDefiner->new(from_step => 1, to_step => 2, from_key => 'chunked_regions_file', to_key => 'chunked_regions_file'), VRPipe::StepAdaptorDefiner->new(from_step => 2, to_step => 3, from_key => 'vcf_split_files', to_key => 'vcf_files'), VRPipe::StepAdaptorDefiner->new(from_step => 2, to_step => 4, from_key => 'vcf_split_files', to_key => 'vcf_files'), VRPipe::StepAdaptorDefiner->new(from_step => 3, to_step => 4, from_key => 'vep_txt', to_key => 'vep_txt'), VRPipe::StepAdaptorDefiner->new(from_step => 4, to_step => 5, from_key => 'conseq_vcf', to_key => 'vcf_files'), VRPipe::StepAdaptorDefiner->new(from_step => 5, to_step => 6, from_key => 'concat_vcf', to_key => 'vcf_files'),],
