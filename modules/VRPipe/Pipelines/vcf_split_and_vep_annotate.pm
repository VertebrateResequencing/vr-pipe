...
class VRPipe::Pipelines::vcf_split_and_vep_annotate with VRPipe::PipelineRole {
    ...
    method adaptor_definitions {
        (
            ...
            { from_step => 2, to_step => 3, from_key => 'vcf_split_files', to_key => 'vcf_files' },
            { from_step => 2, to_step => 4, from_key => 'vcf_split_files', to_key => 'vcf_files' },
            { from_step => 3, to_step => 4, from_key => 'vep_txt', to_key => 'vep_txt' },
            { from_step => 4, to_step => 5, from_key => 'conseq_vcf', to_key => 'vcf_files' },
            { from_step => 5, to_step => 6, from_key => 'concat_vcf', to_key => 'vcf_files' },
        );
    }
    
    method behaviour_definitions {
        (
            { after_step => 6, behaviour => 'delete_outputs', act_on_steps => [2, 3, 4], regulated_by => 'cleanup', default_regulation => 1 },
            { after_step => 2, behaviour => 'delete_outputs', act_on_steps => [1], regulated_by => 'remove_input_vcfs', default_regulation => 0 },
