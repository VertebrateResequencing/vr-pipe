...
class VRPipe::Pipelines::bam_2015_vrtrack_processing_no_recal_no_gtcheck with VRPipe::PipelineRole {
    method name {
        return 'bam_2015_vrtrack_processing_no_recal_no_gtcheck';
    }
    
    method description {
        return 'Copy bam files stored in iRODs (found with the irods all_with_warehouse_metadata datasource) to local disc, generate QC stats and graphs, realign around known indels, add NM and BQ tags, does auto QC and updates the VRTrack db. Finally, it archives the bam files. Uses 2015 versions of samtoools, bcftools, picard and GATK.';
    }
    ...
    method adaptor_definitions {
        (
            ...
            { from_step => 13, to_step => 16, from_key => 'headed_bam_files', to_key => 'bam_files' },
            { from_step => 4,  to_step => 16, from_key => 'stats_files', to_key => 'bamcheck_files' },
            { from_step => 13, to_step => 17, from_key => 'headed_bam_files', to_key => 'file' }
        );
    }
