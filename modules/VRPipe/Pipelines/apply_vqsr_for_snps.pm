...
VRPipe::Pipelines::apply_vqsr_for_snps - a pipeline
...
class VRPipe::Pipelines::apply_vqsr_for_snps with VRPipe::PipelineRole {
    method name {
        return 'apply_vqsr_for_snps';
    }
    
    method description {
        return 'Apply VQSR to SNPs.';
    }
    
    method step_names {
        (
            'gatk_apply_recalibration_for_snps', #1
            'bcftools_concat',                   #2 make per-chromosome, VQSR applied vcf files
        );
    }
