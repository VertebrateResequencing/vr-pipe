...
input files are, what their filenames are, nor where their output files will go.
The B<VRPipe> system will take care of all of this for the Step automatically.
The system also handles other things, like making sure the command line ran
successfully (exited with 0), and that the input files were available, and that
the output files were actually created.
...
not even name it 'call_snps', because in the future you may want to call SNPs in
a completey different way, and you can't update the Step without affecting
...
Job using the C<new_requirements()> method. This is just a sensible default;
it can be overridden by the user or by the system when appropriate.
