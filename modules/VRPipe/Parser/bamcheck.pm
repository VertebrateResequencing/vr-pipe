    ...
    method _get_header {
        ...
        while (<$fh>) {
                ...
                $_ =~ /^COV\s+(\S+)\s+(\d+)\s+(\d+)/ )
            {
                ...
                $self->throw(
"'$key' sections are not understood! Is this really a bamcheck file?"
        ...
        if ( $cov_count > 0 ) {
            $self->_mean_coverage(
                sprintf( "%0.2f", $cov_total / $cov_count ) );
        }
        ...
        $self->throw(
"Unable to parse all SN lines (only saw $saw) - is this a bamcheck file?"
        );
    }
}
