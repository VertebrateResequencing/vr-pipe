...
main objects users are likely to think about. Users want to "run a pipeline"
on their data. The pipelines that they can run are defined by these objects. A
...
C<VRPipe::Pipelines::[your_pipeline_name]>. Special tricks automatically convert
these into VRPipe::Pipeline (and associated) objects in the database.
...
of a pipeline is safe. The advice is, test and stabalise a Pipeline, and once it
is in production never change it. If it turns out there are changes to steps
