...
Updates bam metadata following lanelet Genotype consistency check using htscmd,
setting library_gt = 1 for confirmed bams.
    ...
    method analyse_htscmd_output (ClassName|Object $self: Str|File :$gt_file_path!, Str|File :$source_bam!) {
        ...
        if ($expected eq $gtype1 || ($expected eq $gtype2 && $score1 == $score2)) {
            ...
            foreach my $bam_path (@bam_files) {
                ...
                next
                  if ($meta->{targeted_bases_mapped} && $meta->{targeted_bases_mapped} == 0)
