    ...
    method body_sub {
        return sub {
            ...
            foreach my $input_bam (@{ $self->inputs->{bam_files} }) {
                ...
                my $profile  = $self->output_file(sub_dir => $sub_dir, output_key => 'text_files', basename => "$sample", type => 'txt', metadata => { $sample_key => $sample });
                my $logfile  = $self->output_file(sub_dir => $sub_dir, output_key => 'text_files', basename => "$sample.log", type => 'txt', metadata => { $sample_key => $sample });
                my $pdfile   = $self->output_file(sub_dir => $sub_dir, output_key => 'plot_file', basename => "$sample.pdf", type => 'any', metadata => { $sample_key => $sample });
