...
calculates the intersection of the control bed file with those of the query
files.
...
class VRPipe::Steps::cnv_control_comparison with VRPipe::StepRole {
    ...
    method inputs_definition {
        return {
            bed_files => VRPipe::StepIODefinition->create(
                type        => 'txt',
                max_files   => -1,
                description => 'Reformatted bed file from CNV analyses of query samples (non-control)'
            )
        };
    }
    ...
    method body_sub {
        return sub {
            ...
            foreach my $bed_file (@{ $self->inputs->{bed_files} }) {
                ...
                if ($meta->{$control_key}) {
                    $control_path = $bed_path;
                }
                else {
                    push(@query_paths, $bed_path);
                }
            }
            foreach my $query_path (@query_paths) {
                my $query_file         = VRPipe::File->get(path => $query_path);
                my $intersect_basename = $query_file->basename . '.intersection';
                my $diff_basename      = $query_file->basename . '.diff';
                my $intersect_file     = $self->output_file(output_key => 'intersect_file', basename => "$intersect_basename", type => 'txt', metadata => $query_file->metadata);
                my $intersect_path     = $intersect_file->path;
                my $diff_file          = $self->output_file(output_key => 'diff_file', basename => "$diff_basename", type => 'txt', metadata => $query_file->metadata);
                my $diff_path          = $diff_file->path;
                my $this_cmd           = "$bedtools_exe intersect -u -f 0.5 -r -a $query_path -b $control_path > $intersect_path && $bedtools_exe intersect -v -f 0.5 -r -a $query_path -b $control_path > $diff_path";
    ...
    method outputs_definition {
        return {
            intersect_file => VRPipe::StepIODefinition->create(
                type        => 'txt',
                description => 'Bed file containing the intersection of control and query bed files',
                max_files   => -1,
                min_files   => 0
            ),
    ...
    method description {
        return "Calculates the intersection between bed control and query cnv files";
    }
    ...
    method bed_intersection (ClassName|Object $self: Str $cmd_line) {
        my ($cnv_query_path, $intersection_path, $diff_path) = $cmd_line =~ /^.* -a (\S+) .* > (\S+) .* (\S+)$/;
        
        syquery($cmd_line) && $self->throw("failed to run [$cmd_line]");
        
        my $cnv_query_file = VRPipe::File->get(path => $cnv_query_path);
        my $cnv_query_lines = $cnv_query_file->lines;
        $cnv_query_file->disconnect;
        ...
        if ($combined_lines == $cnv_query_lines) {
            ...
            $cnv_query_file->add_metadata($new_meta);
            return 1;
        }
        else {
            $self->warn("Number of intersection and diff file lines do not equal the original query cnv line number: $cnv_query_path.");
            return 0;
        }
    }
}
