...
class VRPipe::Steps::retroseq_discover with VRPipe::StepRole {
    method options_definition {
        return { 
            ...
            'refTEs_param' => VRPipe::StepOption->get(description => '-refTEs option value, tab file with TE type and BED file of reference elements'),
            'exde_param' => VRPipe::StepOption->get(description => '-exd option value, fofn of BED files of discordant mate exclusion regions'),
        };
    }
    ...
    method body_sub {
        return sub {
            ...
            foreach my $bam_file (@{$self->inputs->{bam_files}}) {
                ...
                my $cmd = "$retroseq_exe -discover -bam $input_path -refTEs $refTEs_param -exd $exde_param -output $output_path";
				$self->warn($cmd);
