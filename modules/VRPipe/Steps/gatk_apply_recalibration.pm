...
class VRPipe::Steps::gatk_apply_recalibration extends VRPipe::Steps::gatk {
    ...
    method inputs_definition {
        return { vcf_files => VRPipe::StepIODefinition->get(type => 'bin', max_files => -1, description => 'one or more tabixed vcf files processed by VariantRecalibrator'),
			recal_files => VRPipe::StepIODefinition->get(type => 'txt', max_files => -1, description => 'a recalibration table file in CSV format for each input vcf', metadata => {source_vcf => 'the vcf file from which calibrations were calculated'}),
			tranches_files => VRPipe::StepIODefinition->get(type => 'txt', max_files => -1, description => 'a tranches file generated by VariantRecalibrator for each vcf', metadata => {source_vcf => 'the vcf file from which calibrations were calculated'}),
		};
    }
