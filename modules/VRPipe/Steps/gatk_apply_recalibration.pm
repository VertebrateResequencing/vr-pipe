...
class VRPipe::Steps::gatk_apply_recalibration extends VRPipe::Steps::gatk {
    ...
    method inputs_definition {
        return {
            ...
            recalibration_file => VRPipe::StepIODefinition->create(type => 'recal',    max_files => 2,  description => 'a recalibration table file in CSV format for each input vcf'),
            tranches_file      => VRPipe::StepIODefinition->create(type => 'tranches', max_files => 2,  description => 'a tranches file generated by VariantRecalibrator for each vcf'),
        };
    }
    ...
    method body_sub {
        return sub {
            ...
            foreach my $recal (@{ $self->inputs->{recalibration_file} }) {
                ...
                $recal_file = $recal if ($meta->{mode} && $meta->{mode} eq $mode);
            }
            $self->throw("No input recalibration file with mode metadata $mode found") unless $recal_file;
            
            my $tranches_file;
            foreach my $tranches (@{ $self->inputs->{recalibration_file} }) {
                my $meta = $tranches->metadata;
                $tranches_file = $tranches if ($meta->{mode} && $meta->{mode} eq $mode);
            }
