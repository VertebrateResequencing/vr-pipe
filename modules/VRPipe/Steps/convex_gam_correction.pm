...
Runs GAMCorrectionPerSample R script from the CoNVex package, generating GAM Correction files from Read Depth and L2R files
	...
	method body_sub {
		return sub {
			...
			foreach my $rd_file (@{$self->inputs->{rd_files}}) {
                ...
                my $cmd = "R --vanilla --slave --args '$l2r_path,$features_file,$gam_path,$rd_path,$breakpoints_file' < $rscript_path/GAMCorrectionPerSample.R";
