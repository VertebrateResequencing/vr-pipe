...
class VRPipe::Steps::lane_bam_reheader with VRPipe::StepRole {
    ...
    method inputs_definition {
        return { bam_files => VRPipe::StepIODefinition->get(type => 'bam',
                                                            max_files => -1,
                                                            description => '1 or more bam files to correct headers for',
    ...
    method outputs_definition {
        return { reheaded_bam_files => VRPipe::StepIODefinition->get(type => 'bam',
                                                                     max_files => -1,
                                                                     description => 'sam file(s) with good headers and RG tags',
                                                                     metadata => {lane => 'lane name (a unique identifer for this sequencing run, aka read group)',
                                                                                  library => 'library name',
                                                                                  sample => 'sample name',
                                                                                  center_name => 'center name',
    ...
    method description {
        return "Modifies the header of a lane bam file to have complete sequence information, a good RG line, chained PG lines, and ensures all records have an RG tag";
    }
}
...
@RG     ID:1    PL:ILLUMINA     PU:110206_IL22_05794_8  LB:UK10K_THY5003268 1791473     PI:170  DS:Study UK10K_RARE_THYROID  DT:2011-02-06T00:00:00+0000     SM:UK10K_THY5003268     CN:SC
