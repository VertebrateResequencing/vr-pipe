...
class VRPipe::Steps::mpileup_bcf with VRPipe::StepRole {
    method options_definition {
        return { samtools_exe => VRPipe::StepOption->get(description => 'path to samtools executable', optional => 1, default_value => 'samtools'),
                 samtools_mpileup_options => VRPipe::StepOption->get(description => 'samtools mpileup options excluding -f', optional => 1, default_value => '-EDSV -C50 -m2 -F0.0005 -d 10000 -P ILLUMINA -g'),
    ...
    method body_sub {
        return sub {
            ...
            $self->dispatch_wrapped_cmd('VRPipe::Steps::mpiluep_bcf', 'mpileup_bcf_and_check', [$cmd, $req, {output_files => [$bcf_file]}]); 
