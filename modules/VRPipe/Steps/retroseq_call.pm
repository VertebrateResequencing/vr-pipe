...
class VRPipe::Steps::retroseq_call with VRPipe::StepRole {
    method options_definition {
        return { 
            ...
            retroseq_filter => VRPipe::StepOption->get(description => '-filter option, tab file with TE type and BED file of reference elements to filter out'),
    ...
    method body_sub {
        return sub {
            ...
            foreach my $bam_file (@{$self->inputs->{bam_files}}) {
                ...
                $basename =~ s/\.bam$/.rseq.vcf/;

				my $rseq_vcf = $self->output_file(output_key => 'rseq_vcf', basename => $basename, type => 'vcf');
				my $output_path = $rseq_vcf->path;

                my $cmd = "$retroseq_exe -call -bam $bam_path -input $bed_files{$bam_path} -ref $retroseq_ref -filter $retroseq_filter $retroseq_call_options -output $output_path";
				$self->warn($cmd);
    ...
    method outputs_definition {
        return { rseq_vcf => VRPipe::StepIODefinition->get(type => 'txt',
                                                               description => 'retroseq VCF of TE calls',
                                                               max_files => -1) };
    }
