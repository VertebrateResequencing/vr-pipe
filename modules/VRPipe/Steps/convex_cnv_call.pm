...
class VRPipe::Steps::convex_cnv_call extends VRPipe::Steps::r_script {
    ...
    method inputs_definition {
        return { gam_files => VRPipe::StepIODefinition->create(type => 'txt', max_files => -1, description => '1 or more gam files from which to call CNV', metadata => { sample => 'sample name' }) };
    }
    ...
    method body_sub {
        return sub {
            ...
            foreach my $gam_file (@{ $self->inputs->{gam_files} }) {
                ...
                my $cnv_file = $self->output_file(output_key => 'cnv_files', basename => $basename, type => 'txt', metadata => $gam_file->metadata);
    ...
    method outputs_definition {
        return { cnv_files => VRPipe::StepIODefinition->create(type => 'txt', max_files => -1, description => 'a CNV call file for each input GAM file', metadata => { sample => 'sample name' }), };
    }
