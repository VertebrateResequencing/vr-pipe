...
class VRPipe::Steps::gatk_variant_recalibration extends VRPipe::Steps::gatk {
    around options_definition {
        return { %{ $self->$orig },
                 variant_recalibration_options => VRPipe::StepOption->create(description => '-resource options for training the gaussian mixture model in the GATK VariantRecalibrator'),
    ...
    method outputs_definition {
        return { recalibration_file      => VRPipe::StepIODefinition->create(type => 'txt', max_files => 1, description => 'Variant recalibration data file'),
                 tranches_file           => VRPipe::StepIODefinition->create(type => 'txt', max_files => 1, description => 'Variant recalibration tranches file'),
                 tranches_plot_file      => VRPipe::StepIODefinition->create(type => 'any', max_files => 1, description => 'Plots to visualise the recalibration data'),
                 ...
                 recalibration_plot_file => VRPipe::StepIODefinition->create(type => 'any', max_files => 1, description => 'PDF file associated with rscript file.') };
    }
