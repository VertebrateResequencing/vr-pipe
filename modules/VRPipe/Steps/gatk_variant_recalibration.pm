...
class VRPipe::Steps::gatk_variant_recalibration extends VRPipe::Steps::gatk {
    ...
    method body_sub {
        return sub {
            ...
            my $recal_file         = $self->output_file(output_key => 'recalibration_file',       basename => "$mode.recal",              type => 'recal',    metadata => { mode => $mode });
            my $recal_index_file   = $self->output_file(output_key => 'recalibration_index_file', basename => "$mode.recal.idx",          type => 'bin',      metadata => { mode => $mode });
            my $tranches_file      = $self->output_file(output_key => 'tranches_file',            basename => "$mode.recal.tranches",     type => 'tranches', metadata => { mode => $mode });
            my $tranches_plot_file = $self->output_file(output_key => 'tranches_plot_file',       basename => "$mode.recal.tranches.pdf", type => 'pdf',      metadata => { mode => $mode });
            my $rscript_file       = $self->output_file(output_key => 'rscript_file',             basename => "$mode.recal.r",            type => 'txt',      metadata => { mode => $mode });
            my $recal_plot_file    = $self->output_file(output_key => 'recalibration_plot_file',  basename => "$mode.recal.r.pdf",        type => 'pdf',      metadata => { mode => $mode });
    ...
    method outputs_definition {
        return {
            recalibration_file       => VRPipe::StepIODefinition->create(type => 'recal',    max_files => 1, description => 'Variant recalibration data file'),
            recalibration_index_file => VRPipe::StepIODefinition->create(type => 'bin',      max_files => 1, description => 'Variant recalibration data file'),
            tranches_file            => VRPipe::StepIODefinition->create(type => 'tranches', max_files => 1, description => 'Variant recalibration tranches file'),
            tranches_plot_file       => VRPipe::StepIODefinition->create(type => 'pdf',      max_files => 1, description => 'Plots to visualise the recalibration data', check_existence => 0),
            ...
            recalibration_plot_file  => VRPipe::StepIODefinition->create(type => 'pdf',      max_files => 1, description => 'PDF file associated with rscript file.', check_existence => 0)
        };
    }
