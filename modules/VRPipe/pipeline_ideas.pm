...
=head1 VRPipe::Persistent
 ...
 my @object_array;
 VRPipe::Persistent::capture_start(\@object_array);
 # ... use classes that inherit from VRPipe::Persistent ...
...
=head1 VRPipe::FileTypeHandler*
 ...
 # VRPipe::FileTypeHandler::<filetype> modules implement
...
=head1 VRPipe::File
 ...
 unless ($file_obj->num_records) {
    ...
    my $handler = VRPipe::FileTypeHandler->new(file => $file_obj);
...
=head1 VRPipe::JobManager::Submission
 ...
 # forms the key.
 my $sub = VRPipe::JobManager::Submission->new(
                job => $VRPipe_jobmanager_job,
 ...
 VRPipe::JobManager::Submission::scheduler($VRPipe_jobmanager_scheduler);
...
=head1 VRPipe::JobManager::Scheduler*
 ...
 my $args_string = $sch->args($VRPipe_jobmanager_requirements);
...
=head1 VRPipe::PipelineManager::DataSource
 ...
 a VRPipe::Pipelines::* module will run on a single element from that list.
 ...
 bam files, and a particular VRPipe::Pipelines::* module might be capable of
 ...
 # another example:
 $dat = VRPipe::PipelineManager::DataSource->new(type => 'Fofn',
                source => '/abs/path/to/my.fofn',
                method => 'all_files');
 # and another example:
 $dat = VRPipe::PipelineManager::DataSource->new(type => 'Chain',
                source => $VRPipe_pipelinemanager_setup_obj,
 ...
 # used as the instance-specific-input to at least 1 VRPipe::Pipelines::*
 # module.
...
=head1 VRPipe::PipelineManager::Setup
 ...
 my $set = VRPipe::PipelineManager::Setup->new(name => 'chimp_mapping',
                module => 'VRPipe::Pipelines::Mapping',
                data_source => $VRPipe_pipelinemanager_datasource,
...
=head1 VRPipe::Pipeline*
 ...
 # possible.
 # Also, the idea is to change each VRPipe::Pipelines::* module so that it does
 # a single well defined thing (so VRPipe::Pipelines::Mapping does mapping
 # only, not recalibration), and to chain different Setups together. So you'd
 # create a Setup where module => 'VRPipe::Pipelines::Mapping' and data_source
...
=head1 VRPipe::PipelineManager::Step
 ...
 if ($act->complete) {
    ...
    if (@submissions) {
        ...
        # this turns the submission objects into a VRPipe::PersistentArray
...
=head1 Random Notes
 ...
 DBIx::Simple is nicer than using DBI
